...
//!
//! HWPX is implemented in `polaris-dvc-hwpx`. The HWP 5.0 legacy binary format
...
pub enum ParseError {
    ...
    Hwpx(#[from] polaris_dvc_hwpx::HwpxError),
...
#[derive(Debug)]
pub enum Document {
    Hwpx(polaris_dvc_hwpx::HwpxDocument),
}

pub fn parse(bytes: &[u8]) -> Result<Document, ParseError> {
    match sniff(bytes) {
        DocumentFormat::Hwpx => Ok(Document::Hwpx(polaris_dvc_hwpx::open_bytes(bytes)?)),
