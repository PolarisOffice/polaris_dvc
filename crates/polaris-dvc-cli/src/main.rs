...
impl OutputOptionArg {
    fn to_core(self) -> polaris_dvc_core::output::OutputOption {
        use polaris_dvc_core::output::OutputOption as O;
        match self {
...
fn main() -> ExitCode {
    ...
    if matches!(format, Format::Xml) && cli.dvc_strict {
        eprintln!("polaris-dvc: --format=xml is not yet implemented");
        return ExitCode::from(2);
    }

    let Some(input) = cli.input else {
        eprintln!("polaris-dvc: input path is required");
        return ExitCode::from(2);
    };
    ...
    let doc_bytes = match read_input(&input) {
        Ok(b) => b,
        Err(e) => {
            eprintln!("polaris-dvc: failed to read input: {e}");
            return ExitCode::from(3);
        }
    };

    let doc = match polaris_dvc_format::parse(&doc_bytes) {
        Ok(polaris_dvc_format::Document::Hwpx(d)) => d,
        Err(e) => {
            eprintln!("polaris-dvc: parse error: {e}");
            return ExitCode::from(3);
        }
    };

    let spec = match cli.spec {
        Some(p) => match polaris_dvc_core::rules::loader::load_spec_from_path(&p) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("polaris-dvc: failed to load spec: {e}");
                return ExitCode::from(2);
            }
        },
        None => polaris_dvc_core::rules::schema::RuleSpec::default(),
    };

    let opts = polaris_dvc_core::engine::EngineOptions {
        stop_on_first: cli.simple,
        profile: if cli.dvc_strict {
            polaris_dvc_core::engine::CheckProfile::DvcStrict
        } else {
            polaris_dvc_core::engine::CheckProfile::Extended
        },
    ...
    if let Some(path) = cli.file {
        if let Err(e) = std::fs::write(&path, body.as_bytes()) {
            eprintln!("polaris-dvc: failed to write output: {e}");
            return ExitCode::from(3);
        }
    } else {
