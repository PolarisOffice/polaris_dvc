...
use polaris_dvc_core::engine::{validate, EngineOptions};
...
fn full_pipeline_detects_expected_violations() {
    ...
    let doc = polaris_dvc_hwpx::open_bytes(&bytes).expect("parse synthetic HWPX");
...
fn full_pipeline_json_output_shape() {
    let bytes = build_hwpx();
    let doc = polaris_dvc_hwpx::open_bytes(&bytes).unwrap();
