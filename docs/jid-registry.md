...
`crates/polaris-core/src/jid_registry.rs` holds a `pub const JID_*:
...
The curated `jid` submodule in `crates/polaris-core/src/error_codes.rs`
...
`crates/polaris-core/tests/jid_registry_drift.rs` runs on every
...
1. Find the upstream constant in the generated file, e.g.
   `JID_TABLE_SIZEWIDTH` (`crates/polaris-core/src/jid_registry.rs`).
2. Add a short-name alias in the `jid` module of
   `crates/polaris-core/src/error_codes.rs`:
   ```rust
...
4. Reference the alias from the engine's checker (e.g.,
   `crates/polaris-core/src/engine.rs::check_table`).
